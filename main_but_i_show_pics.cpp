...
struct finder {
    vector<double> sins, coss;

    finder() : sins(180), coss(180) {
        for (int i = 0; i < 180; i++) {
...
// stack overflow  copypaste
    ...
    Mat
    get_intersections(Mat &borders, Mat &hor, Mat &vert, int imglen0, int le,
    ...
    find_corners(const Mat &source, const Mat &borders, const Mat &fft_v, const Mat &fft_h, const Mat &points,
        ...
        auto verify = [&clusters, this](line l) {
            int score = 0;
            int r = l.r, th = l.th;
        ...
        vector<cv::Point2f> target = {{0.0,                         0.0},
                                      {0.0,                         static_cast<float>(imglen0)},
                                      {static_cast<float>(imglen0), static_cast<float>(imglen0)},
                                      {static_cast<float>(imglen0), 0.0}};
        ...
        auto activity = [&](const Mat &x, bool flag) {
            ...
            auto hor = Mat(
                    cv::abs(x(cv::Rect(0, len, width, height - len)) - x(cv::Rect(0, 0, width, height - len))) >
                    threshold);
 ...
 */
    ...
    vector<cv::Point2f> find_corners(const string &filename) {
        ...
        return find_corners(img_grayscale);
    }

    vector<cv::Point2f> find_corners(Mat &img_grayscale) {
        Mat img_128;
        ...
        cv::resize(coloured, coloured, {imglen*2, imglen*2});
